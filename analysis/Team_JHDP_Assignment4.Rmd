...
According to @davidlasswell, when searching for a job, cost of living should be taken into account. It also relates to the question whether the salary of the job would be enough to cover the living cost in a particular state. Therefore, in this subsection, we would observe the living cost in the state capital cities in Australia. It is also because these cities relatively have more job listing than other small cities. 
...
bow("https://www.numbeo.com/cost-of-living/country_result.jsp?country=Australia")

cost <- bow("https://www.numbeo.com/cost-of-living/country_result.jsp?country=Australia")
...
cost_of_living_state <- cost_of_living %>% mutate(
  state = case_when(
...
```{r livingcost_state_map_join}

#read the oz_map.rds data
oz_map <- read_rds(here::here("data/oz_map.rds"))

#join the cost_of_living_state with oz_map
living_cost_map <- cost_of_living_state %>%
  # filter Newcastle and Gold Coast
  # in order to get the detailed picture in Sydney and Brisbane
  # if these cities are not filtered, 
  # then the living cost map will display the average of living cost in NSW and QLD
  filter(city != "Newcastle",
         city != "Gold Coast") %>%
  full_join(oz_map, c("state" = "state_abb"))
```

```{r map, fig.cap="Living cost in Australia by State Capital City}

ggplot(living_cost_map,
       aes(x = longitude,
           y = latitude,
           geometry = geometry,
           fill = living_cost_incld_rent)) + 
  geom_sf() + 
  scale_fill_viridis_c()+
  labs(fill = "Living cost index include rent cost") +
  theme(legend.position = "bottom")

```


Figure \ref{fig:map} shows that Sydney was found to be the city with the highest living cost, followed by Melbourne, Canberra, and Queensland. Western Australia and Tasmania had a lower living cost compared to those cities. The figure also conveys that Adelaide was the city with the lowest living cost in Australia. 

It also interesting to observe whether there is a correlation, between the living cost in those cities and the number of job listings in there. Therefore, let us take a look first, to the number of job listings in those cities. 

```{r jobcity, include=FALSE}
