...
```{r}
job_types <- data_cleaned %>% select(job_title, post_date, job_type) %>% mutate(Month = month(post_date, label = TRUE, abbr = FALSE))
```

```{r}
job_type_counts <- job_types %>% mutate(Month = as.character(Month),
...
```{r}
full_time <- job_type_counts %>% filter(job_type == "Full Time") %>% summary()
  ggplot(full_time) +
  geom_boxplot()
```
