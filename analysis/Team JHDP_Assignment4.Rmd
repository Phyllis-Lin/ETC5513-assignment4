...
```{r}
job_type <- data_cleaned %>% select(job_title, post_date, job_type) %>% mutate(Month = month(post_date, label = TRUE, abbr = FALSE))
```

```{r}
job_type_counts <- job_type %>% group_by(Month, job_type) %>% count() 
```

```{r}
job_type_counts %>%
ggplot(aes(x = Month, y = n)) +
  geom_col(aes(fill = job_type)) +
